...
public class SolicitacaoDAO {
    ...
    public ArrayList<Solicitacao> listarSolicitacaoDoLavador(int id) {
        ...
        SolicitacaoState solicitacaoState;
        try {
            PreparedStatement pstmt = conexao.prepareStatement("select s.id as ID_Solicitacao, c.id as ID_Cliente, s.id_avaliacao, c.nome as nome_cliente, e.bairro, e.cidade, s.porte, s.data_solicitacao, s.valor_total, s.status from lavador l, usuario u, solicitacao s, cliente c, endereco e, cliente_endereco ce where c.id = s.id_cliente and u.id = l.id_usuario and l.id = s.id_lavador and ce.id_cliente = c.id and ce.id_endereco = e.id and u.id = ? order by s.data_solicitacao");
            pstmt.setInt(1, id);
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
